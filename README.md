...
- o monitoramento do WhatsApp institucional agora tambem alimenta o `Operational State` com sinais leves de atencao, prazo, resposta e possivel reuniao; isso ajuda o briefing e a leitura curta do dia sem transformar o Atlas numa caixa de entrada automatica
...
- `web.search` ja existe como capability real e passa a ser usada tambem pelo planner em pedidos de informacao recente, comparacao, validacao com fontes e pesquisa inicial de passagens/hospedagem; `maps.route`, `maps.distance`, `maps.tolls` e `maps.places_search` agora podem usar Google Maps/Routes quando `GOOGLE_MAPS_API_KEY` estiver ativo com a API de rotas habilitada, e continuam aparecendo como gap claro quando essa configuracao nao existir
- em fluxos de rota e custo de viagem, o Atlas agora mantem uma missao ativa por chat: respostas parciais como `13 km/l e gasolina 6,70`, `Porto Alegre e Torres`, `ida e volta` ou `deixa isso` continuam o mesmo objetivo em vez de reabrir clarificacoes genericas
- em fluxos de descoberta local, o Atlas tambem consegue manter o contexto curto do chat: pedidos como `me mostra restaurantes perto de mim` podem ficar abertos ate voce completar com `na Restinga`, `em Torres` ou outro ponto de referencia, sem perder a missao no meio
