...
export function buildPersonalOperationalProfileReply(profile: PersonalOperationalProfile): string {
  ...
  return [
    ...
    `- Papel operacional: ${profile.userRole ?? "custom"}`,
    `- Profissão: ${profile.profession ?? "não definida"}`,
    `- Pack profissional: ${profile.professionPackId ?? "nenhum"}`,
    ...
    `- Compartilhamento padrão: ${profile.audiencePolicy?.mode ?? "mixed"} | audiência ${profile.audiencePolicy?.defaultAudience ?? "self"}`,
